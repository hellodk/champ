 ...
 *
 * Encapsulates the logic of reading `aidev.*` configuration values
...
export class ProviderFactory {
  ...
  private async createClaude(
    ...
    const apiKey = await secrets.get("aidev.claude.apiKey");
    return new ClaudeProvider({
      ...this.baseConfig("claude"),
      model:
  ...
  private async createOpenAI(
    ...
    const apiKey = await secrets.get("aidev.openai.apiKey");
    return new OpenAIProvider({
      ...this.baseConfig("openai"),
  ...
  private async createGemini(
    ...
    const apiKey = await secrets.get("aidev.gemini.apiKey");
    return new GeminiProvider({
      ...this.baseConfig("gemini"),
  ...
  private async createVLLM(
    ...
    const apiKey = await secrets.get("aidev.vllm.apiKey");
    return new VLLMProvider({
      ...this.baseConfig("vllm"),
  ...
  private async createOpenAICompatible(
    ...
    const apiKey = await secrets.get("aidev.openaiCompatible.apiKey");
    return new OpenAICompatibleProvider({
      ...this.baseConfig("openai-compatible"),
